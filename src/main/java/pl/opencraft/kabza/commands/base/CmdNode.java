...
public class CmdNode {
    ...
    public void executeCommand(CmdMethodParams params, String label, String[] args) {
        ...
        if (args.length > 0) {
            ...
            params.sender.sendMessage(PREFIX + RED + plugin.messages.get(MessageEnum.CMD_ERROR_ARGUMENT_NOT_VALID) + YELLOW + args[0] + RED + '.' +
                    '\n' + GREEN + plugin.messages.get(MessageEnum.CMD_INFO_TAB_COMPLETE_AVAILABLE));
            return;
        }
        if (!children.isEmpty()) {
            StringBuilder sb = new StringBuilder();
            sb.append(PREFIX).append(RED).append(plugin.messages.get(MessageEnum.CMD_ERROR_COMMAND_INCOMPLETE));
            sb.append('\n').append(WHITE).append(plugin.messages.get(MessageEnum.CMD_INFO_USAGE));
            for (CmdNode child : children) {
            ...
            sb.append('\n').append(GREEN).append(plugin.messages.get(MessageEnum.CMD_INFO_TAB_COMPLETE_AVAILABLE));
